    ...
    public function index()
    {
        try {
			if (isset($this->session->dflogin_admin) ) {
					...
					$this->load->view('masterforms/designation/designation');
					$this->load->view('include/footer');
					$this->load->view('masterforms/designation/designation_script');
			...
			if (isset($this->session->dflogin_admin) ) {
				// if($this->session->dflogin['typeid']==1 || ($this->session->dflogin['typeid']==2 && $this->session->dflogin['entry']==1)) {
					...
					if(isset($request->txtDpname) && preg_match('/^[0-9a-zA-Z ]{0,50}$/',$request->txtDpname)){
						$insert[0]['desgname']=$request->txtDpname;
					}else{
						$data['title']="Alert!";
    ...
    public function r_pc(){
        try{
			if (isset($this->session->dflogin_admin) ) {
					...
					// $where="isactve=1";
					// $companyid=$this->session->dflogin_admin['adminid'];
					// $where="entryby=".$companyid;
					$orderby="id asc";
					$res=$this->Model_Db->select(3,null,null,$orderby);
					if($res){
						foreach ($res as $r){
							$data['status']=true;
							$data['data'][]=array(
