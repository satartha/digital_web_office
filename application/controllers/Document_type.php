    ...
    public function index()
    {
        try {
			// if (isset($this->session->dflogin) && $this->session->dflogin['typeid']<=2) {
					...
					$this->load->view('masterforms/document_type/document_type');
					$this->load->view('include/footer');
					$this->load->view('masterforms/document_type/document_typeScript');
			...
			if (isset($this->session->dflogin) ) {
				// if($this->session->dflogin['typeid']==1 || ($this->session->dflogin['typeid']==2 && $this->session->dflogin['entry']==1)) {
					...
					if(isset($request->txtDpname) && preg_match('/^[0-9a-zA-Z ]{0,50}$/',$request->txtDpname)){
						$insert[0]['typename']=$request->txtDpname;
					}else{
						$data['title']="Alert!";
						$data['message']="Document Type name error";
						$status=false;
					}
							...
							if($request->txtid>0){
								if(isset($this->session->editForm['id']) && $this->session->editForm['id']==$request->txtid){
									...
									if($res!=false){
										...
										$data['title']="Alert!!";
										$data['message']="Document Type updating failed";
										$data['status']=false;
									}
								}else{
									$data['status']=false;
								...
								if($res!=false){
									...
									$data['title']="Alert!";
									$data['message']="Document type creating failed";
									$data['status']=false;
								}
							}else{
