...
<script>
    ...
    // })
	 ...
	 function pdf_rpt() {
		...
		$.ajax({
			type:'post',
			url:"<?=base_url('Staff_dashboard/show_dir')?>",
			dataType:'json',
			success:function (f) {
				var dname=f.path;
				console.log(dname);
				   ...
				   $("#cr_dt").text(f.cr_on);
					...
					if(f.filecount>0){
						...
						// }else{
							...
							download=`<a style="text-decoration:none;" href="<?=base_url('')?>${f.document_p[j].documentpath}" download ><i class="mdi mdi-download" style="font-size: 25px;color:#109bbb"></a></i>&nbsp;&nbsp;`;
                            view=`<a href="<?=base_url('')?>${f.document_p[j].documentpath}" target="_blank" onclick=""><i class="mdi mdi-file" style="font-size: 25px;color:#109bbb"></i></a>`
						// }


						var doc_r_id=f.document_p[j].documentpath;
						var bt=btoa(doc_r_id);
						var dt_nw="<?php echo  base_url('Dashboard/File_dtl?I=')?>"+bt;

						var heading_nw=`<a   onclick="" style="cursor:pointer;">${f.document_p[j].filename}</a>`;
					      
						    htmlfile_nw+=`<tr>
							<td>${ftype}</td>
							<td><b>${heading_nw}</b><br><br>Created on : ${f.document_p[j].entryat}</td>
							<td>File Size :  ${f.meta[j].file_size} <br> Expiry On : ${f.document_p[j].expirydate} <br> <br> Last Modified : ${f.meta[j].last_mod}</td>
							<td>${download}  &nbsp; ${view}</td>
							</tr>`;
	...
	function getsubfolder(dname,id){
		...
		$.ajax({
			type:'post',
			url:"<?=base_url('Staff_dashboard/subname')?>",
			data:{dname:dname},
			dataType:'json',
			success:function (f){
				...
				if(f.status){
					...
					if(f.data.filescount>0){
						...
						var heading_nw=`${f.data.new_file[j].files.heading}`;
					      
						    htmlfile_nw+=`<tr>
							<td>${ftype}</td>
							<td>${heading_nw}<br>Created on : ${f.data.new_file[j].files.create_time}</td>
	...
	$("#frmDirct").submit(function (e){
		...
		$.ajax({
			type:'post',
			url:"<?=base_url('Staff_dashboard/make_directory')?>",
