...
<script>
    ...
    $("#frmEform2").submit(function (e){
        ...
        if(frm!=""){
            $.ajax({
				...
				contentType: false,
				cache: false,
                url:"<?=base_url('Entryform/create_company')?>",
    ...
    function report_candidate() {
		$.ajax({
			...
			type:'post',
			url:'<?=base_url("Entryform/r_candidate")?>',
			dataType:'json',
			success:function (f) {
				var htmldata="";
				var j=0;
				...
				for(var i in f.data){
                    var km=0;
					j++;
					htmldata+=`<tr><td>${j}</td><td>${f.data[i].name}</td><td>${f.data[i].companycode}</td><td>${f.data[i].email}</td><td>${f.data[i].registrationno}</td><td>${f.data[i].gstno}</td><td>${f.data[i].pan}</td><td><img src='<?=base_url("assets/com_logo/")?>${f.data[i].logo}' alt='no image uploaded' height='100' width='100'></td>`;
    ...
    function active_inactive(rowid,isactive) {
		activeInactiveRecords(rowid,isactive,"<?=base_url('Entryform/active_inactive_candidate')?>");
	}
	function edit(id){
		$.ajax({
			type:'post',
			url:'<?=base_url("Entryform/edit_candidate")?>',
			data:{id:id},
			dataType:'json',
			success:function (f) {
                console.log(f);
