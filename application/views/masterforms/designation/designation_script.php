...
<script>
    ...
    function report_pc() {
        $.ajax({
            ...
            success:function (f) {
                ...
                for(var i in f.data){
                    j++;
                    htmldata+=`<tr><td>${j}</td><td>${f.data[i].desgname}</td>`;
    ...
    function active_inactive(rowid,isactive) {
        activeInactiveRecords(rowid,isactive,"<?=base_url('Designation/active_inactive_pc')?>");
    }
    function edit(id){
        // alert(id);
        $.ajax({
            type:'post',
            url:'<?=base_url("Designation/edit_pc")?>',
            data:{id:id},
            dataType:'json',
            success:function (f) {
                console.log(f);
