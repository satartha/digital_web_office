...
<script>
    ...
    $("#frmwp").submit(function (e){
        e.preventDefault();
        var frm = $("#frmwp").serializeToJSON();
        if(frm!=""){
            const frm_data=do_data_list(JSON.stringify(frm));
            $.ajax({
                type:'post',
                url:"<?=base_url('Workflow/c_dp')?>",
    ...
    function report_pc() {
        $.ajax({
            ...
            success:function (f) {
                ...
                for(var i in f.data){
                    j++;
                    htmldata+=`<tr><td>${j}</td><td>${f.data[i].wfnm}</td>`;
    ...
    function active_inactive(rowid,isactive) {
        activeInactiveRecords(rowid,isactive,"<?=base_url('Workflow/active_inactive_pc')?>");
    }
    function edit(id){
        // alert(id);
        $.ajax({
            type:'post',
            url:'<?=base_url("Workflow/edit_pc")?>',
            data:{id:id},
            dataType:'json',
            success:function (f) {
                console.log(f);
