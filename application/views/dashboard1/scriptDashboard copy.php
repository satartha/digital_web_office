		...
		$.ajax({
			...
			success:function (f) {
				if(f.status){
					...
					for(var i in f.data){
						k++;
                        htmldata+=`<li style='list-style-type:none;color:#ff4f00;cursor:pointer;'><i class='mdi mdi-folder-plus'></i> <span id="${k}" onclick="getsubfolder('${f.data[i].directory}',${k},${v})">${f.data[i].directory_name}</span>
